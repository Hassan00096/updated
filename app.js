...
mediaInput.addEventListener(
    ...
    function () {
        ...
        if (selectedMediaType === "image") {
            ...
            videoNotice.hidden = true;
            analyseButton.hidden = false;
            analyseButton.textContent = "Analyse Video";
        }
...
function captureVideoFrameBlob(videoElement) {
    return new Promise(function (resolve, reject) {
        try {
            ...
            canvas.toBlob(function (blob) {
                ...
                else reject(new Error("Could not extract frame from video."));
            }, "image/jpeg", 0.95);
        } catch (err) {
            reject(err);
        }
...
analyseButton.addEventListener(
    ...
    async function () {
        if (!selectedFile) {
            showError(
                "Please select a file before starting analysis."
            );
        ...
        try {
            ...
            if (selectedMediaType === "video") {
                const frameBlob = await captureVideoFrameBlob(videoPreview);
                fileToSend = new File(
                    [frameBlob],
                    selectedFile.name.replace(/\.[^/.]+$/, "") + "_frame.jpg",
                    { type: "image/jpeg" }
                );
            }
        ...
        <section class="page">
            ...
            <section class="report-section">
                ...
                <div class="detail-row">
                    ...
                    <p>
                        Facial region localized and cropped with 25% margin padding for facial landmark evaluation.
                    </p>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <p>
                        Primary facial region (eyes, nose, mouth area) analyzed for synthetic artifacts.
                    </p>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <p>
                        Spatial feature map and deep learning neural embedding check completed.
                    </p>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <p>
                        SHA-256 binary hash and media header integrity verified.
                    </p>
                </div>
...
// Interactive handlers for Feature buttons
const featureBtnImage = document.getElementById("feature-btn-image");
const featureBtnFace = document.getElementById("feature-btn-face");
const featureBtnConfidence = document.getElementById("feature-btn-confidence");
const featureBtnReport = document.getElementById("feature-btn-report");
const featureBtnHistory = document.getElementById("feature-btn-history");
const featureBtnVideo = document.getElementById("feature-btn-video");

if (featureBtnImage) {
    featureBtnImage.addEventListener("click", function () {
...
if (featureBtnFace) {
    featureBtnFace.addEventListener("click", function () {
        document.getElementById("how-it-works").scrollIntoView({ behavior: "smooth" });
    });
}

if (featureBtnConfidence) {
    featureBtnConfidence.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
    });
}

if (featureBtnReport) {
    featureBtnReport.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
    });
}

if (featureBtnHistory) {
    featureBtnHistory.addEventListener("click", function () {
        document.getElementById("history-list").scrollIntoView({ behavior: "smooth" });
    });
}

if (featureBtnVideo) {
    featureBtnVideo.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
        openUploadArea("video");
    });
}

// Interactive handlers for Step cards
...
if (stepCard1) {
    stepCard1.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
        mediaSelection.hidden = false;
        uploadArea.hidden = true;
    });
...
if (stepCard2) {
    stepCard2.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
        if (uploadArea.hidden) {
            openUploadArea("image");
        }
        mediaInput.click();
    });
...
if (stepCard3) {
    stepCard3.addEventListener("click", function () {
        document.getElementById("detector").scrollIntoView({ behavior: "smooth" });
    });
}

if (stepCard4) {
    stepCard4.addEventListener("click", function () {
        document.getElementById("history-list").scrollIntoView({ behavior: "smooth" });
    });
}